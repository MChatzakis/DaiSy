...
struct SofaRangeFixture : public benchmark::Fixture {
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        fprintf(stderr, "[SOFA-RANGE] Before buildIndex (n=%llu dim=%llu).\n",
        ...
        fprintf(stderr, "[SOFA-RANGE] Indexing done. n_query=%llu r=%.2f threads=%d\n",
...
BENCHMARK_DEFINE_F(SofaRangeFixture, BM_Sofa_RangeSearch)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[SOFA-RANGE] dataset=%s threads=%d r=%.2f\n",
        ...
        fprintf(stderr, "[SOFA-RANGE] Done (n_query=%llu r=%.2f).\n",
                (unsigned long long)n_query, r);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(SofaRangeFixture, BM_Sofa_RangeSearch)
