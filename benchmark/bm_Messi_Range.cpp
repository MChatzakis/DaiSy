...
struct MessiRangeFixture : public benchmark::Fixture {
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        fprintf(stderr, "[MESSI-RANGE] Before buildIndex (n=%llu dim=%llu).\n",
        ...
        fprintf(stderr, "[MESSI-RANGE] Indexing done. n_query=%llu r=%.2f threads=%d\n",
...
BENCHMARK_DEFINE_F(MessiRangeFixture, BM_Messi_RangeSearch)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[MESSI-RANGE] dataset=%s threads=%d r=%.2f\n",
        ...
        fprintf(stderr, "[MESSI-RANGE] Done (n_query=%llu r=%.2f).\n",
                (unsigned long long)n_query, r);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(MessiRangeFixture, BM_Messi_RangeSearch)
