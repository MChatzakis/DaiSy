...
struct FreshSearchOnlyFixture : public benchmark::Fixture {
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        fprintf(stderr, "[FRESH] Before buildIndex (n_database=%zu dim=%zu).\n", n_database_u, dim_u);
        fflush(stderr);
        ...
        fprintf(stderr, "[FRESH] Indexing finished (n_database=%zu dim=%zu).\n", n_database_u, dim_u);
        fflush(stderr);
        ...
        fprintf(stderr, "[FRESH] n_database=%zu n_query=%zu dim=%zu k=%zu threads=%d\n",
                n_database_u, (size_t)n_query, dim_u, k, config.thread_count);
        fflush(stderr);
    }

    void TearDown(const benchmark::State&) override {
        fprintf(stderr, "[FRESH] TearDown start.\n");
        fflush(stderr);
        delete search;
        delete[] database;
        delete[] query;
        ...
        I = nullptr;
        D = nullptr;
        fprintf(stderr, "[FRESH] TearDown done.\n");
        fflush(stderr);
    }
};

BENCHMARK_DEFINE_F(FreshSearchOnlyFixture, BM_Fresh_SearchOnly)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[FRESH] --- Query phase ---\n");
        fprintf(stderr, "[FRESH]   dataset=%s  n_database=%zu\n", dataset_name.c_str(), n_database);
        fprintf(stderr, "[FRESH]   search_threads=%d  n_query=%zu  k=%zu\n", thread_count, (size_t)n_query, k);
        fflush(stderr);
        search->searchIndex(query, n_query, static_cast<daisy::idx_t>(k), I, D);
        fprintf(stderr, "[FRESH] Querying finished (n_query=%zu k=%zu).\n", (size_t)n_query, k);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(FreshSearchOnlyFixture, BM_Fresh_SearchOnly)
