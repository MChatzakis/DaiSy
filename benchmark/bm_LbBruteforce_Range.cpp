...
struct LbBruteforceRangeFixture : public benchmark::Fixture {
    daisy::LbBruteforce* search   = nullptr;
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        search = new daisy::LbBruteforce(daisy::DistanceType::L2_SQUARED);
        search->setNumThreads(config.thread_count);

        fprintf(stderr, "[LB-RANGE] Before buildIndex (n=%llu dim=%llu).\n",
        ...
        fprintf(stderr, "[LB-RANGE] Indexing done. n_query=%llu r=%.2f threads=%d\n",
...
BENCHMARK_DEFINE_F(LbBruteforceRangeFixture, BM_LbBruteforce_RangeSearch)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[LB-RANGE] dataset=%s threads=%d r=%.2f\n",
        ...
        fprintf(stderr, "[LB-RANGE] Done (n_query=%llu r=%.2f).\n",
                (unsigned long long)n_query, r);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(LbBruteforceRangeFixture, BM_LbBruteforce_RangeSearch)
