...
struct SingRangeFixture : public benchmark::Fixture {
    daisy::Sing* search   = nullptr;
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        search = new daisy::Sing(daisy::DistanceType::L2_SQUARED);
        search->setNumThreads(config.thread_count);

        fprintf(stderr, "[SING-RANGE] Before buildIndex (n=%llu dim=%llu).\n",
        ...
        fprintf(stderr, "[SING-RANGE] Indexing done. n_query=%llu r=%.2f threads=%d\n",
...
BENCHMARK_DEFINE_F(SingRangeFixture, BM_Sing_RangeSearch)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[SING-RANGE] dataset=%s threads=%d r=%.2f\n",
        ...
        fprintf(stderr, "[SING-RANGE] Done (n_query=%llu r=%.2f).\n",
                (unsigned long long)n_query, r);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(SingRangeFixture, BM_Sing_RangeSearch)
