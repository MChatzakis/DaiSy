...
struct BruteforceRangeFixture : public benchmark::Fixture {
    daisy::BruteForceSearch* search   = nullptr;
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        search = new daisy::BruteForceSearch(daisy::DistanceType::L2_SQUARED);
        search->setNumThreads(config.thread_count);

        fprintf(stderr, "[BF-RANGE] Before buildIndex (n=%llu dim=%llu).\n",
        ...
        fprintf(stderr, "[BF-RANGE] Indexing done. n_query=%llu r=%.2f threads=%d\n",
...
BENCHMARK_DEFINE_F(BruteforceRangeFixture, BM_Bruteforce_RangeSearch)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[BF-RANGE] dataset=%s threads=%d r=%.2f\n",
        ...
        fprintf(stderr, "[BF-RANGE] Done (n_query=%llu r=%.2f).\n",
                (unsigned long long)n_query, r);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(BruteforceRangeFixture, BM_Bruteforce_RangeSearch)
