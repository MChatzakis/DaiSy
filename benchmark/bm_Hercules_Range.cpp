...
struct HerculesRangeFixture : public benchmark::Fixture {
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        daisy::HerculesConfig hcfg;
        hcfg.index_dir        = "/tmp/hercules_range_bm_" + config.name +
                                "_r" + std::to_string((int)config.r_value) +
                                "_t" + std::to_string(config.thread_count);
        hcfg.num_build_threads = config.thread_count;

        search = new daisy::Hercules(daisy::DistanceType::L2_SQUARED, hcfg);
        search->setNumThreads(config.thread_count);

        fprintf(stderr, "[HERCULES-RANGE] Before buildIndex (n=%llu dim=%llu).\n",
        ...
        fprintf(stderr, "[HERCULES-RANGE] Indexing done. n_query=%llu r=%.2f threads=%d\n",
...
BENCHMARK_DEFINE_F(HerculesRangeFixture, BM_Hercules_RangeSearch)(benchmark::State& state) {
    for (auto _ : state) {
        fprintf(stderr, "[HERCULES-RANGE] dataset=%s threads=%d r=%.2f\n",
        ...
        fprintf(stderr, "[HERCULES-RANGE] Done (n_query=%llu r=%.2f).\n",
                (unsigned long long)n_query, r);
        fflush(stderr);
    }
}

BENCHMARK_REGISTER_F(HerculesRangeFixture, BM_Hercules_RangeSearch)
