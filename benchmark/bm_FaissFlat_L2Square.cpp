...
struct FaissFlatFixture : public benchmark::Fixture {
    float* xb = nullptr;
    float* xq = nullptr;
    ...
    void SetUp(const benchmark::State& state) override {
        ...
        if (!parseFilenameForConfig(dataset_filename, "bruteForce", dim, nb, __, ___)) {
            return;
        }

        daisy::idx_t dim_q, nq_val, ____, _____;
        if (!parseFilenameForConfig(query_filename, "bruteForce", dim_q, nq_val, ____, _____)) {
            return;
        }
        if (dim != dim_q) return;
...
BENCHMARK_DEFINE_F(FaissFlatFixture, BM_FaissFlat)(benchmark::State& state) {
    for (auto _ : state) {
        index->search(nq, xq, k, D, I);
    }
}

BENCHMARK_REGISTER_F(FaissFlatFixture, BM_FaissFlat)
    ->Arg(0)
    ->MinTime(2.0)
