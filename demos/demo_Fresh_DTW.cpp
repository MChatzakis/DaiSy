...
int main(){
    ...
    daisy::Fresh fresh_search(daisy::DistanceType::DTW);
    fresh_search.setNumThreads(4);
    fresh_search.setWarpingWindow(std::max(1, static_cast<int>(dim * 0.1)));

    fresh_search.buildIndex(database, n_database, dim);

    daisy::idx_t *I = new daisy::idx_t[n_query * k];
    float *D = new float[n_query * k];
    fresh_search.searchIndex(query, n_query, k, I, D);
