// Fresh range search: report every series within squared-L2 distance r of a query.
...
int main()
{
    ...
    daisy::Fresh fresh_search(daisy::DistanceType::L2_SQUARED);
    fresh_search.setNumThreads(4);
    fresh_search.buildIndex(database, n_database, dim);

    std::vector<std::vector<daisy::idx_t>> I;
    std::vector<std::vector<float>> D;
    fresh_search.searchIndex(query, n_query, config, I, D);
    reportRange("Fresh", r, I, D);
    ...
    printf("\nAll queries match: %s\n",
           matchesBruteforce("Fresh", I, gt_I) ? "yes" : "no");
