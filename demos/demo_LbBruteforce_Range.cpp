// LbBruteforce range search: report every series within squared-L2 distance r of a query.
...
int main()
{
    ...
    daisy::LbBruteforce lb_search(daisy::DistanceType::L2_SQUARED);
    lb_search.setNumThreads(4);
    lb_search.buildIndex(database, n_database, dim);

    std::vector<std::vector<daisy::idx_t>> I;
    std::vector<std::vector<float>> D;
    lb_search.searchIndex(query, n_query, config, I, D);
    reportRange("LbBruteforce", r, I, D);
    ...
    printf("\nAll queries match: %s\n",
           matchesBruteforce("LbBruteforce", I, gt_I) ? "yes" : "no");
