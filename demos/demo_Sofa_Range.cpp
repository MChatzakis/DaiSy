// Sofa range search: report every series within squared-L2 distance r of a query.
...
int main()
{
    ...
    daisy::Sofa sofa_search(daisy::DistanceType::L2_SQUARED);
    sofa_search.setNumThreads(4);
    sofa_search.buildIndex(database, n_database, dim);

    std::vector<std::vector<daisy::idx_t>> I;
    std::vector<std::vector<float>> D;
    sofa_search.searchIndex(query, n_query, config, I, D);
    reportRange("Sofa", r, I, D);
    ...
    printf("\nAll queries match: %s\n",
           matchesBruteforce("Sofa", I, gt_I) ? "yes" : "no");
