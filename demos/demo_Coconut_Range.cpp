// Coconut range search: report every series within squared-L2 distance r of a query.
...
int main()
{
    ...
    coconut.setNumThreads(4);
    coconut.buildIndex(database, n_database, dim);

    std::vector<std::vector<daisy::idx_t>> I;
    std::vector<std::vector<float>> D;
    coconut.searchIndex(query, n_query, config, I, D);
    reportRange("Coconut", r, I, D);
    ...
    printf("\nAll queries match: %s\n",
           matchesBruteforce("Coconut", I, gt_I) ? "yes" : "no");
