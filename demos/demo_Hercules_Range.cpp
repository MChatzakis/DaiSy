// Hercules range search: report every series within squared-L2 distance r of a query.
...
int main()
{
    ...
    // Hercules keeps its leaf payloads on disk, so it needs a writable index directory.
    daisy::HerculesConfig hercules_config;
    hercules_config.index_dir = "/tmp/hercules_range_demo";

    daisy::Hercules hercules_search(daisy::DistanceType::L2_SQUARED, hercules_config);
    hercules_search.setNumThreads(4);
    hercules_search.buildIndex(database, n_database, dim);

    std::vector<std::vector<daisy::idx_t>> I;
    std::vector<std::vector<float>> D;
    hercules_search.searchIndex(query, n_query, config, I, D);
    reportRange("Hercules", r, I, D);
    ...
    printf("\nAll queries match: %s\n",
           matchesBruteforce("Hercules", I, gt_I) ? "yes" : "no");
