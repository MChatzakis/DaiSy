...
namespace daisy
{
    ...
    // Index-build worker data (one per thread)
    struct SOFA_index_worker
    {
        ...
        float              norm_factor;
        bool               is_norm;
        // FFTW (per-thread resources, created before pthread_create)
        float             *ts_buf;
    ...
    // Executes plan, normalises, extracts paa_segments FFT coefficients.
    // Caller must fill ts_buf before calling (plan holds a pointer to it).
    static void sofa_fft_from_ts(
    ...
    // Discretise FFT transform into a SAX-style SFA word using the learned bins.
    static void sofa_sfa_from_fft(
    ...
    // Lower-bound distance contribution for one SFA segment.
    ...
    // Full SFA lower bound between a query FFT vector and one iSAX node.
        ...
        sax_type max_bit_card, int max_card, int n_segments, bool is_norm)
    {
        ...
        int i = 0;
        if (!is_norm) {
            // DC coefficient: real part only, factor 1.0
    ...
    // Evaluates all time series in a leaf node against the query.
    // Full-buffer entries: evaluated directly with exact L2.
    // Partial-buffer entries: SFA lower bound applied first, then exact L2.
    ...
    void *SOFA_topk_search_worker(void *rfdata)
    {
        ...
        // Phase 1: traverse all root subtrees, insert candidate leaves
        while (1) {
        ...
        // Phase 2: pop candidates from own queue, evaluate leaves
        query_result *n;
        while (1) {
            ...
            if (n->distance > bsfdist || n->distance > minimum_distance) break;
            if (n->node->is_leaf)
    ...
    void *sofa_index_creation_worker(void *transferdata)
    {
        ...
        // Signal end of insert phase; wait for all workers before FBL flush
        pthread_barrier_wait(d->lock_barrier1);
        pthread_barrier_wait(d->lock_barrier2);

        // Phase 2: flush first-buffer-layer into tree leaves
    ...
    // Descend the tree using the SFA word of the query to find the nearest leaf.
    ...
    // Refines the best-so-far using the SFA lower bound on all root nodes.
    ...
    void Sofa::sfaBinsInit()
    {
        ...
        binsv = new float[(size_t)paa_segments * (sax_cardinality - 1)];
        for (int i = 0; i < paa_segments; ++i) {
            bins[i] = new float[sax_cardinality - 1];
            for (int j = 0; j < sax_cardinality - 1; ++j)
                bins[i][j] = FLT_MAX;
        }
        for (int j = 0; j < paa_segments * (sax_cardinality - 1); ++j)
            binsv[j] = FLT_MAX;
    }
    ...
    void Sofa::sfaFreeBins()
    {
        ...
        delete[] binsv;        binsv        = nullptr;
        delete[] coefficients; coefficients = nullptr;
    }
    ...
    void Sofa::sfaSetBins()
    {
        ...
        // Last worker takes any remaining records
        ...
        // Split each coefficient's sampled values into breakpoints
        ...
        // Sync flat binsv layout (enables potential future SIMD lower-bound)
        for (int i = 0; i < paa_segments; i++)
            for (int j = 0; j < sax_cardinality - 1; j++)
                binsv[(size_t)i * (sax_cardinality - 1) + j] = bins[i][j];
    ...
    void Sofa::buildIndex(DataSource *data_source)
    {
        ...
        // Learn SFA bins from a sample of the database before indexing
        sfaBinsInit();
        sfaSetBins();
        ...
        // Participate in both barriers so workers can synchronise phases
    ...
    pqueue_bsf Sofa::sofaSearchTopkL2Squared(float *ts, float *fft, node_list *nodelist, idx_t k)
    {
        ...
        pthread_t        tids[search_workers];
        SOFA_workerdata  workerdata[search_workers];
        ...
        for (int i = 0; i < n_pqueue; i++) pqueue_free(allpq[i]);
        free(allpq);
    ...
    void Sofa::searchIndex(const float *query, idx_t n_query, idx_t k, idx_t *I, float *D)
    {
