...
namespace diNoLib
{
    ...
    void LbBruteforce::buildIndex(const float *database, const idx_t n_database, const idx_t dim)
    {
        ...
        for (idx_t dbi = 0; dbi < n_database * dim; dbi++)
        {
            ...
            if (sax_from_ts(vi_vec, this->db_sax_representations[dbi], this->index->settings->ts_values_per_paa_segment, this->index->settings->paa_segments,
                            this->index->settings->sax_alphabet_cardinality, this->index->settings->sax_bit_cardinality) == SUCCESS)
            {
                // success here!
            }
            else
