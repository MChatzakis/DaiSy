...
#include "../hodyssey/indexing.hpp"  // for print_index_stats, CommunicationModuleData, DynamicSchedulingMode, call_module
...
namespace diNoLib
{
    ...
    void Odyssey::searchIndex(const float *query, const idx_t n_query, const idx_t k, idx_t *I, float *D)
    {
        ...
        // Build query array from in-memory buffer (no file)
        ...
        // timer_start(timer_manager, "QA");  // uncomment when TimerManager is available
        // timer_start(timer_manager, "QA_PREPRO");
        odyssey_preprocess_and_sort_queries(this, queries, q_num, true);
        // timer_stop(timer_manager, "QA_PREPRO");
        ...
        // Branch by distance: rest (while, workstealing, cleanup, copy to I,D) is in L2/DTW
    ...
    {
        constexpr bool ENABLE_PRINTS_PER_QUERY = false;  // set to true for per-query debug prints
        const float minimum_distance = FLT_MAX;
        ...
        while (1)
        {
            if (my_rank == rep_find_coordinator_node_rank(replication_data, my_rank))
            {
                if (mode == DynamicSchedulingMode::PERIODIC_CHECK || mode == DynamicSchedulingMode::STANDALONE_THREAD)
                {
                    ...
                    result.total_time = 0.0;  // no timer_manager
    ...
    // and its starting offset in the global file. EKOSMAS helpers
    // (initialize_pRecBuf_ekosmas, index_creation_sequence_worker) are
    // implemented in iSAXIndex.cpp and odyssey/indexing.cpp respectively.
    // ============================================================================
    ...
    void odyssey_optimize_params(Odyssey *odyssey)
    {
        ...
        // ========================================================================
        //
        // 1. Only KNN is supported; no threshold-based BSF-sharing branch.
        //
    ...
    void odyssey_prepare_structures(Odyssey *odyssey, const char *raw_filename)
    {
        ...
        // Log replication groups info (always, not just when verbose)
        if (odyssey->my_rank == 0)
        {
    ...
    void odyssey_log_parameters(Odyssey *odyssey)
    {
        if (odyssey->my_rank == 0 && odyssey->verbose)  // MASTER = 0
        {
            ...
            // Convert mode, scheduling to indices for array access (only KNN supported)
            ...
            // Only DISABLED (0) and S_WS (1) are supported
            int ws_type_idx = static_cast<int>(odyssey->workstealing_data.ws_type);
            if (ws_type_idx < 0 || ws_type_idx >= 2) ws_type_idx = 0;  // Default to disabled
