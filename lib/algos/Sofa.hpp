...
// bringing the data to each worker thread
    ...
    // search phase worker thread function
...
// sofa indez (SFA (DFT) + MDB bins layered on iSAX for top-k L2 search)
    class Sofa : public SimilaritySearchAlgorithm
    {
        // sofa index (SFA wrapper around iSAX)
        // provide index construction and search functions
    private:
        int  read_block_length = 100000; // # threads/blowk during indexing
        int  search_workers = 4; // # threads to use (search)
        int  index_workers  = 2; // # threads to use (index))
        int  n_pqueue = 42; // # per-thread queues
        ...
        int  coeff_number = 0;
        bool is_norm = false;
        // bins/coefficinets declarations 
        void sfaBinsInit();
        void sfaSetBins();
        void sfaFreeBins();
